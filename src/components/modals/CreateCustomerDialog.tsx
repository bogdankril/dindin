...
export default function CreateCustomerDialog({
  ...
  // This useCallback will create the autocomplete instance.
  const setupAutocomplete = useCallback(() => {
    if (!isMapsApiReady || !addressInputRef.current) {
        return;
    };
    
    // Create a new Autocomplete instance and attach it to the input field.
  ...
  // Effect to run when the dialog opens or the map API becomes ready.
  useEffect(() => {
    if (isOpen && isMapsApiReady) {
      setupAutocomplete();
    }
  }, [isOpen, isMapsApiReady, setupAutocomplete]);

  // Effect to clean up when the dialog closes.
  useEffect(() => {
    return () => {
