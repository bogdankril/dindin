...
export default function UsersList() {
  ...
  useEffect(() => {
    ...
    const usersQuery = query(
      collection(db, `artifacts/${APP_ID}/users/${currentUserProfile.companyId}/users`)
    );
  ...
  const handleRoleChange = async (userIdToChange: string, newRole: UserRole) => {
    if (!db || !currentUserProfile?.companyId) return;
    const userDocRef = doc(db, `artifacts/${APP_ID}/users/${currentUserProfile.companyId}/users`, userIdToChange);
  ...
  const handleDeleteUser = (userToDelete: UserProfile) => {
    ...
    showCustomConfirmModal(
        `Are you sure you want to permanently delete the user ${userToDelete.email}? This will remove them from the app but will not delete their login credentials. This action cannot be undone.`,
        () => {
            const userDocRef = doc(db, `artifacts/${APP_ID}/users/${currentUserProfile.companyId}/users`, userToDelete.id);
            deleteDocumentNonBlocking(userDocRef);
            showAppModal("User record deleted successfully.");
        }
    );
  };
